...
faiss_index, kb_docs             = load_kb(
    str(BASE_DIR / "rag" / "agribot_faiss.index"),
    str(BASE_DIR / "rag" / "agribot_docs.pkl"),
)
...
async def ask_endpoint(query: Query):
    ...
    if query.mode == "base":
        ...
        if chunks:
            ...
            answer = generate(prompt, finetuned_model, finetuned_tok, use_prefix=False)
        else:
            answer = FALLBACK_MSG
        ...
        if not is_farming_related(q):
            ...
            if not chunks:
                ...
                prompt = build_rag_prompt(q, chunks)
                raw    = generate(prompt, finetuned_model, finetuned_tok, use_prefix=False)
